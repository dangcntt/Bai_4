...
namespace Project.Net8.Controllers.Major
{
  ...
  ///  [Authorize]
    public class BaiThiController : DefaultReposityController<BaiThiModel>
    {
        ...
        public BaiThiController(DataContext context,  IBaiThiService  service,  IHttpContextAccessor httpContextAccessor) 
        ...
        [HttpPost]
        [Route("create-lich-su")]
        public async Task<IActionResult> CreateLichSu([FromBody] LichSuTuyenTruyenModel model)
        {
            try
            {

                var data = await _service.CreateLichSu(model);
                return Ok(
                    new ResultMessageResponse()
                        .WithData(data)
                        .WithCode(DefaultCode.SUCCESS)
