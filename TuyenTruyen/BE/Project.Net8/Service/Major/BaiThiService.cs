...
namespace Project.Net8.Service.Major
{
    public class BaiThiService : BaseService, IBaiThiService
    {
        ...
        private BaseMongoDb<LichSuTuyenTruyenModel, string> BaseMongoDbLichSu;
            ...
            base(context, contextAccessor)
        {
            ...
            BaseMongoDbLichSu = new BaseMongoDb<LichSuTuyenTruyenModel, string>(_context.LICHSUTUYENTRUYEN);
        }
        ...
        public async Task<dynamic> CreateLichSu(LichSuTuyenTruyenModel model)
        {
            try
            {
                if (model == default)
                ...
                var result = await BaseMongoDbLichSu.CreateAsync(model);
                if (result.Entity.Id == default || !result.Success)
                {
                    throw new ResponseMessageException()
                        .WithException(DefaultCode.CREATE_FAILURE);
